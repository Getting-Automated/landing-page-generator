...
function Validate-SslCertificate {
    ...
    if ([string]::IsNullOrEmpty($CERTIFICATE_ARN)) {
        Write-Message "error" "No certificate ARN found."
        return $false
    }
    ...
    try {
        ...
        if ($CertDetails.Status -ne "ISSUED") {
            ...
            $CnameRecords = $CertDetails.Certificate.DomainValidationOptions.ResourceRecord
        ...
        Write-Message "error" "Error validating SSL certificate: $_"
        return $false
    }
}
...
function Check-DnsPropagation {
    ...
    for ($i = 1; $i -le $maxAttempts; $i++) {
        try {
            $result = dig +short $DOMAIN_NAME

            if ($result -match $CLOUDFRONT_DOMAIN) {
                Write-Message "success" "DNS propagation complete. $DOMAIN_NAME is pointing to the CloudFront distribution."
                return $true
            } else {
                Write-Message "info" "DNS changes have not propagated yet. Attempt $i of $maxAttempts."
                if ($i -lt $maxAttempts) {
                    Write-Message "info" "Waiting $attemptDelay seconds before next check..."
                    Start-Sleep -Seconds $attemptDelay
                }
            }
        } catch {
            Write-Message "error" "Error checking DNS: $_"
            return $false
        }
    }

    Write-Message "info" "DNS changes have not propagated yet."
    return $false
}
...
function Request-AndValidateCertificate {
    ...
    try {
        ...
        $script:CERTIFICATE_ARN = $CertArn
        ...
        Write-Message "error" "Error in Request-AndValidateCertificate: $_"
        Write-Message "error" "Stack trace: $($_.ScriptStackTrace)"
        return "failed"
    }
}
...
function Create-OrGetCloudFrontDistribution {
    ...
    if ([string]::IsNullOrEmpty($ExistingDist)) {
        ...
        $DistributionConfig | Out-File -FilePath $TempFile -Encoding utf8

        $Distribution = aws cloudfront create-distribution --distribution-config file://$TempFile --profile $AWS_CLI_PROFILE | ConvertFrom-Json
    ...
    Set-Variable -Name "CLOUDFRONT_DISTRIBUTION_ID" -Value $CLOUDFRONT_DISTRIBUTION_ID -Scope Script

    # After creating or getting the CloudFront distribution
...
# Function to provide manual DNS info (update this function as well)
function Provide-ManualDnsInfo {
    Write-Message "info" "To point your domain to this website, create the following DNS records at your DNS provider:"
    Write-Host ""
    Write-Host "1. For the root domain ($DOMAIN_NAME):"
    Write-Host "   Type: A"
    Write-Host "   Name: @"
    Write-Host "   Value: Set up an ALIAS record pointing to $CLOUDFRONT_DOMAIN"
    Write-Host ""
    Write-Host "2. For the www subdomain (www.$DOMAIN_NAME):"
    Write-Host "   Type: CNAME"
    Write-Host "   Name: www"
    Write-Host "   Value: $CLOUDFRONT_DOMAIN"
    Write-Host ""

    # Only show SSL certificate validation info if the certificate is not yet issued
    $CertStatus = aws acm describe-certificate --certificate-arn $CERTIFICATE_ARN --region us-east-1 --query 'Certificate.Status' --output text --profile $AWS_CLI_PROFILE
    if ($CertStatus -ne "ISSUED") {
        ...
        Write-Message "warning" "Please update your DNS records as instructed above, including the SSL certificate validation records."
...
function Check-ExistingResources {
    ...
    $CertArns = aws acm list-certificates --region us-east-1 --query "CertificateSummaryList[?DomainName=='$DOMAIN_NAME'].CertificateArn" --output text --profile $AWS_CLI_PROFILE
    if ([string]::IsNullOrEmpty($CertArns) -or $CertArns -eq "None") {
        $CERT_EXISTS = $false
        $CERTIFICATE_ARN = $null
    } else {
        $CERT_EXISTS = $true
        $CERTIFICATE_ARN = ($CertArns -split '\s+')[0]  # Take the first ARN if multiple are returned
...
function Get-ValidCertificate {
    Write-Message "info" "Checking for valid SSL certificates..."
    
    try {
        $Certificates = aws acm list-certificates --region us-east-1 --query "CertificateSummaryList[?Status=='ISSUED']" --output json --profile $AWS_CLI_PROFILE | ConvertFrom-Json

        foreach ($Cert in $Certificates) {
            $CertDetails = aws acm describe-certificate --certificate-arn $Cert.CertificateArn --region us-east-1 --query 'Certificate.{Domains:DomainValidationOptions[].DomainName,Status:Status}' --output json --profile $AWS_CLI_PROFILE | ConvertFrom-Json

            $domainValid = $CertDetails.Domains -contains $DOMAIN_NAME -or $CertDetails.Domains -contains "www.$DOMAIN_NAME"
            
            if ($domainValid -and $CertDetails.Status -eq "ISSUED") {
                Write-Message "success" "Valid certificate found for $DOMAIN_NAME"
                Write-Message "info" "Certificate ARN: $($Cert.CertificateArn)"
                Write-Message "info" "Certificate is valid for: $($CertDetails.Domains -join ', ')"
        ...
        Write-Message "error" "Error checking for valid certificates: $_"
        return $null
    }

    Write-Message "info" "No valid certificate found for $DOMAIN_NAME"
    return $null
}
...
try {
    ...
    $ValidCertArn = Get-ValidCertificate
    if ($ValidCertArn) {
        $script:CERTIFICATE_ARN = $ValidCertArn
        Write-Message "info" "Using existing valid SSL certificate: $CERTIFICATE_ARN"
    } else {
        $certStatus = Request-AndValidateCertificate
        if ($certStatus -eq "complete") {
            $script:CERTIFICATE_CREATED = $true
            Write-Message "success" "New certificate created and validated successfully."
        } elseif ($certStatus -eq "paused") {
            $script:deploymentPaused = $true
            Write-Message "info" "Certificate creation initiated. Manual DNS configuration required."
        } else {
    ...
    if ($deploymentComplete) {
        ...
        if ($DNS_METHOD -eq "route53") {
            ...
            if (-not (Check-DnsPropagation)) {
                ...
                Write-Message "info" "DNS is already correctly configured. No updates needed."
            }
        } elseif ($DNS_METHOD -eq "manual") {
            if (-not (Check-DnsPropagation)) {
                Write-Message "info" "Manual DNS configuration required."
                Provide-ManualDnsInfo
            } else {
                Write-Message "info" "DNS is already correctly configured. No manual updates needed."
            }
        } else {
